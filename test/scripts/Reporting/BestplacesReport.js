	...
	function(evt){
		...
		if(activeBars[0] !== undefined){
			...
			$.ajax({
				...
				method : "POST",
				data : {label : activeBars[0].label,StartYear:StartYear,EndYear:EndYear,report_type:'BestPlaces'},
				success : function(responce){
								$('#segment-modal-title').text('Dolphin Sighted in '+activeBars[0].label);
								$('#segment-modal-body').html(responce);
								$('#segment-model').modal('show');
							}	
			});
		}
	}
