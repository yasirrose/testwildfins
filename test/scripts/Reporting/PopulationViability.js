	...
	/*$("#bar-chart").click( 
			function(evt){
				...
				if(activeBars[0] !== undefined){
					...
					$.ajax({
						url : "http://test.wildfins.org/Modules/Reporting/getBarChartSpecificDataByYears.cfm",
						method : "POST",
						data : {label : activeBars[0].label,fromYear:activeBars[0].label},
						success : function(responce){
										$('#year-modal-title').text('Dolphin Sighted in '+activeBars[0].label);
										$('#year-modal-body').html(responce);
										$('#year-model').modal('show');
									}	
					});
				}
			}
	...
	function(evt){
		...
		if(activePoints[0] !== undefined){
			...
			$.ajax({
					...
					success : function(responce){
								$('#all-modal-title').text('All '+activePoints[0].label+'s');
								$('#all-modal-body').html(responce);
								$('#all-model').modal('show');
							}
				});
		}
	}
...
if(all_sex_pieData[0].value == 0 && all_sex_pieData[1].value == 0 && all_sex_pieData[2].value == 0 && calves_sex_pieData[0].value == 0 && calves_sex_pieData[1].value == 0 && calves_sex_pieData[2].value == 0 && juvenile_sex_pieData[0].value == 0 && juvenile_sex_pieData[1].value == 0 && juvenile_sex_pieData[2].value == 0 && adult_sex_pieData[0].value == 0 &&adult_sex_pieData[1].value == 0 &&adult_sex_pieData[2].value == 0 && adult_sex_pieData[0].value == 0 && unknown_sex_pieData[0].value == 0 && unknown_sex_pieData[1].value == 0 && unknown_sex_pieData[2].value == 0){
	$('#pie_panel_body').html('<h4>No Record Found.</h4>');
}
...
var bar_chart = new Chartv2(bar_ctx, {
    type: 'bar',
    data: {
        labels: sightingYear,
        datasets: [
        ...
        },
		{
            label: 'Adult',
            data: AdultData,
						backgroundColor: "rgba(253, 180, 92, 0.7)",
						hoverBackgroundColor: "rgba(253, 180, 92, 0.7)",
						hoverBorderWidth: 2,
						hoverBorderColor: 'lightgrey'
        },
		{
            label: 'Unknown',
            data: UnknownData,
						backgroundColor: "rgba(83, 237, 83, 0.7)",
						hoverBackgroundColor: "rgba(83, 237, 83, 0.7)",
						hoverBorderWidth: 2,
						hoverBorderColor: 'lightgrey'
        },
        ]
