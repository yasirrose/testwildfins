...
var DolphinByYearChart = {
    labels : dolphinByYear,
    datasets : [
        {
            data :dolphinSeen
        }
...
var DolphinBySegmentChart = {
    labels : dolphinBySegment,
    datasets : [
        {
            data :dolphinSeenBySegment
        }
    ]
};


if(dolphinByYear.length){
	var handleDolphinByYearGraph = function(animationOption) {
		var animationOption = (animationOption) ? animationOption : false;
		var ctx2 = document.getElementById('dolphinByYear_Chart').getContext('2d');
		
		ctx2.canvas.height = 193;
		
		 window.DolphinByYearGraph = new Chart(ctx2).Bar(DolphinByYearChart, {
			animation: animationOption,
			responsive : true
		});
		DolphinByYearGraph.datasets[0].bars[0].fillColor = "green";
		DolphinByYearGraph.update();
	}
	handleDolphinByYearGraph();
}


if(dolphinBySegment.length){
	var handleDolphinBySegmentGraph = function(animationOption) {
		var animationOption = (animationOption) ? animationOption : false;
		var ctx2 = document.getElementById('bodyConditionBySegment_Chart').getContext('2d');
		
		ctx2.canvas.height = 193;
		
		 window.DolphinBySegmentGraph = new Chart(ctx2).Bar(DolphinBySegmentChart, {
			animation: animationOption,
			responsive : true
		});
		DolphinBySegmentGraph.datasets[0].bars[0].fillColor = "red";
		DolphinBySegmentGraph.update();
	}
	handleDolphinBySegmentGraph();
}
});
