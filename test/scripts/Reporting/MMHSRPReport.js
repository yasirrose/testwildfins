...
var ChartDataForDeathBySegment = {
    labels : dolphinsCodeBySegment,
    datasets : [
        {
            data :dolphinsDeathBySegment
        }
    ]
};
<!-- For Chart1 Data --->
...
if(dolphinsCode.length){
	var handleGenerateGraphByDeathAge = function(animationOption) {
		var animationOption = (animationOption) ? animationOption : false;
		var ctx2 = document.getElementById('barChartDeathAge').getContext('2d');
		
		ctx2.canvas.height = 193;
		
		 window.barChart = new Chart(ctx2).Bar(ChartDataForDeathAge, {
			animation: animationOption,
			responsive : true
		});
		barChart.datasets[0].bars[0].fillColor = "orange";
		barChart.update();}
	handleGenerateGraphByDeathAge();
}
var handleGenerateGraphByDeathBySegment = function(animationOption) {
	var animationOption = (animationOption) ? animationOption : false;
	var ctx2 = document.getElementById('barChartDeathBySegment').getContext('2d');
	
    ctx2.canvas.height = 193;
	
     window.barChart = new Chart(ctx2).Bar(ChartDataForDeathBySegment, {
        animation: animationOption,
		responsive : true
    });
...
//	 barChart.datasets[0].bars[6].fillColor = " #28ABE3";
     barChart.update();
	 $("#barChartDeathBySegment").click( 
			function(evt){
				var activeBars = barChart.getBarsAtEvent(evt);
				var startYear  = $("#deathBySegmentStartYear").val();
				var endYear    = $("#deathBySegmentEndYear").val();
				if(activeBars[0] !== undefined){
					console.log(activeBars[0].label);
					$.ajax({
						url : "http://test.wildfins.org/Modules/Reporting/getBarChartDeathBySegments.cfm",
						method : "POST",
						data : {label : activeBars[0].label,startYear:startYear,endYear:endYear},
						success : function(responce){
										$('#segments-modal-title').text('Dolphins By Segment '+activeBars[0].label);
...
handleGenerateGraphByDeathBySegment();
