function deleteRecord(id) {
	...
	if (result == true) {
		$.ajax({
			url:application_root+"StaticData.cfc?method=DeleteMissDescriptors",
			type : "get",
			data : {id : id},
			async:true,
...
function updateRecord(id) {
	$('#MissDescriptors').val($('#MissDescriptors-'+id).text());
	$("#add").attr('name', 'editMissDescriptors');
	$("#add").text('Edit');
	$("#MissDescriptors_id").val(id);
