function deleteRecord(id) {
	...
	if (result == true) {
		$.ajax({
			url:application_root+"StaticData.cfc?method=DeleteHitLocation",
			type : "get",
			data : {id : id},
			async:true,
...
function updateRecord(id) {
	$('#HitLocationAbbreviation').val($('#HitLocationAbbreviation-'+id).text());
	$('#Description').val($('#Description-'+id).text());
	$("#add").attr('name', 'editHitLocation');
	$("#add").text('Edit');
	$("#HitLocation_id").val(id);
