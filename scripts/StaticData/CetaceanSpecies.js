function deleteRecord(id) {
    bootbox.confirm("Are you sure?", function(result) {
        ...
        if (result == true) {
            $.ajax({
                url: application_root + "StaticDataNew.cfc?method=DeleteCetaceanSpecies",
                type: "get",
                data: {
                    id: id
                },
...
function updateRecord(id) {
    $('#CetaceanSpeciesName').val($('#cam-' + id).text());
    $("#add").attr('name', 'editCetaceanSpecies');
    $("#add").text('Edit');
    $("#CetaceanSpecies_id").val(id);
